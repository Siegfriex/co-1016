// CuratorOdyssey Firebase Functions 에뮬레이터 API 테스트
// P1 Alex Chen - 실제 작동 검증

const axios = require('axios');

// 현재 작동 중인 에뮬레이터 포트들
const EMULATOR_PORTS = [8777, 8142, 8449, 8793, 8825];
const BASE_URL = `http://localhost:${EMULATOR_PORTS[0]}`; // 첫 번째 포트 사용

console.log('🚀 CuratorOdyssey Firebase Functions 에뮬레이터 테스트 시작');
...
async function testArtistSummary() {
  try {
    ...
    const response = await axios.get(`${BASE_URL}/getArtistSummary?id=ARTIST_0005`);
...
async function testTimeseries() {
  try {
    console.log('\n📈 3. Timeseries 테스트...');
    const response = await axios.get(`${BASE_URL}/getArtistTimeseries?id=ARTIST_0005&axis=제도`);
...
async function testAIReport() {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/generateAiReport`, {
...
async function testAllEndpoints() {
  console.log('🔧 P1 Alex Chen - Firebase Functions 에뮬레이터 검증');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  if (successCount === totalTests) {
    ...
    console.log('🚀 Firebase Functions 에뮬레이터 준비 완료');
  } else {
